...
def MiniBatchGD(trainX, trainY, validationX, validationY, testX, testY, GDparams, W1, W2, b1, b2, lamb):
    ...
    plotAcc(GDparams, accs_train, accs_valid, lamb)
...
def plotAcc(GDparams, accs_train, accs_valid, lamb): # Update steps
    t = np.arange(0, len(accs_train), 1)
    fig, ax = plt.subplots()
    ax.plot(t, accs_train, label = 'Training')
    ax.plot(t, accs_valid, label = 'Validation')
    ax.legend()
    ax.set(xlabel='Update steps', ylabel='Accuracy', title='Lambda = ' + str(lamb))
    plt.show()
...
def plotEtas(etas):
    ...
    fig, ax = plt.subplots()
    ax.plot(t, etas)
    ax.set(xlabel='Update steps', ylabel='eta')
    plt.show()
...
def main():
    ...
    # Testa med n_btach 100 och epcohs 48

    # n_batch = 100
    # n_epochs = 10
    # n_s = 500
    # Train Accuracy: 0.6331    Valid Accuracy: 0.4348
    ...
    #[W1, W2, b1, b2] = MiniBatchGD(trainX[:,0:20], trainY[:,0:20], validationX[:,0:1000], validationY[:,0:1000], GDparams, W1, W2, b1, b2, lamb)
    l_min = -5
    l_max = -1
