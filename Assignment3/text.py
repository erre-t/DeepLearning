...
class RNN():
    def __init__(self):
        ...
        self.m = [self.d, 50, self.K] # nodes in the hidden layer
        ...
        for i in range(1, self.k+1):
            ...
            self.gamma.append(np.random.normal(0, np.sqrt(2 / self.m[i-1]), (self.m[i],1)))
            self.beta.append(np.zeros((self.m[i],1)))
        ...
        self.run_batch_normalization = True
        self.training = True
        self.testing = False

        self.avg_mu = [0]*self.k
        self.avg_v = [0]*self.k
    ...
    def EvaluateClassifier(self, X):
        ...
        if self.run_batch_normalization:
            s_list = []
            s_hat_list = []
            s_til_list = []
            x_list = []
            ...
            for l in range(self.k):
                s = np.dot(self.W[l], x) + self.b[l]

                if self.training:
                    mu = np.mean(s, axis=1, keepdims=True)
                    v = np.var(s, axis=1, keepdims=True) / n
                    self.avg_mu.append(self.alpha * self.avg_mu[l] + (1 - self.alpha) * mu)
                    self.avg_v.append(self.alpha * self.avg_v[l] + (1 - self.alpha) * v)
                ...
                print(s.shape)
                print(mu.shape)
                s_hat = (s - mu)/np.sqrt(v + self.eps) # Batch Normalization
                s_til = np.multiply(self.gamma[l], s_hat) + self.beta[l]
                x = np.maximum(0, s_hat)

                s_list.append(s)
                s_hat_list.append(s_hat)
                s_til_list.append(s_til)
                mu_list.append(mu)
                v_list.append(v)
                x_list.append(x)

            P = softmax(np.dot(self.W[self.k-1], s_list[self.k-2]) + self.b[self.k-1])

            return P, x_list #, s_list, s_hat_list, s_til_list, mu_list, v_list
            ...
            P = softmax(np.dot(self.W[self.k-1], x_list[self.k-2]) + self.b[self.k-1])
