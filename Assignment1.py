...
def MiniBatchGD(trainX, trainY, validationX, validationY, testX, testY, GDparams, W, b, lamb):
    ...
    plotAcc(GDparams, accs_train, accs_valid, acc_test)
...
def plotAcc(GDparams, accs_train, accs_valid, acc_test): # Update steps
    t = np.arange(0, len(accs_train), 1)
    fig, ax = plt.subplots()
    ax.plot(t, accs_train, label = 'Training')
    ax.plot(t, accs_valid, label = 'Validation')
    ax.legend()
    ax.set(xlabel='Update steps', ylabel='Accuracy', title= 'Test Accuracy: ' + str(acc_test), label = ['Traning', 'Validation'])
    plt.show()
